...
func TestAsInt32FromInt64(t *testing.T) {
	tests := []struct {
		name     string
		input    int64
		expected int32
		...
		{
			name:     "typical timestamp delta (100ms)",
			input:    100,
			expected: 100,
		},
		{
			name:     "typical timestamp delta (1 second)",
			input:    1000,
			expected: 1000,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := AsInt32FromInt64(tt.input)
			require.Equal(t, tt.expected, result)
		})
	}
}

func TestAsInt32FromInt64_Constants(t *testing.T) {
	// Test using math constants to ensure we handle the exact boundaries
	require.Equal(t, int32(math.MaxInt32), AsInt32FromInt64(math.MaxInt32))
	require.Equal(t, int32(math.MinInt32), AsInt32FromInt64(int64(math.MinInt32)))
	require.Equal(t, int32(math.MaxInt32), AsInt32FromInt64(int64(math.MaxInt32)+1))
	require.Equal(t, int32(math.MinInt32), AsInt32FromInt64(int64(math.MinInt32)-1))
}
